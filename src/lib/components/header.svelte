<script lang="ts">
	import { getThemeContext } from '$lib/context.svelte';
	...
	function toggleTheme() {
		if (themeStore.value == 'dark') themeStore.value = 'light';
		else themeStore.value = 'dark';
	}
</script>
...
{#snippet themeController()}
	<label class="swap swap-rotate">
		...
		<!-- sun icon -->
		<svg
			class="swap-off h-7 w-10 fill-current"
		...
		<!-- moon icon -->
		<svg
			class="swap-on h-7 w-10 fill-current"
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 24 24"
		>
			<path
